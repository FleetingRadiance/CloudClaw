...
function getProtectedFileRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('cloudclow_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('cloudclow_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into cloudclow_tenant, leaving authenticated file
...
/**
 * 将下载/文件地址规范化为「当前服务器同源」地址。
 *
 * 后端在 SYSTEM_AES_KEY / JWT_SECRET 未配置、或存储后端返回内网地址时，
 * 可能给出绝对内网地址（如 http://minio:9000/...、http://backend:8080/...）。
 * 这类地址浏览器无法访问。此函数：
 *   - 相对路径（/api/...）原样返回，由浏览器基于当前源解析（正确）；
 *   - 绝对地址但 host 与当前页不同（内网地址）→ 仅保留 path+query+hash，
 *     改写为当前页面的 origin，使下载走当前服务器；
 *   - 同源绝对地址原样返回。
 */
