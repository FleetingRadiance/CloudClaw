...
instance.interceptors.request.use(
  (config) => {
    ...
    // 但这条优化会被任何把 cloudclow_tenant 写成激活租户的代码（OIDC
...
instance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry && !originalRequest.url?.includes('/auth/refresh')) {
      ...
      const refreshToken = localStorage.getItem('cloudclow_refresh_token');
      ...
      if (refreshToken) {
        try {
          ...
          if (response.success && response.data) {
            ...
            localStorage.setItem('cloudclow_token', token);
            localStorage.setItem('cloudclow_refresh_token', newRefreshToken);
