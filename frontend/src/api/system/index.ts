...
/** Minimal user info for the LDAP user list. */
...
/**
 * List all LDAP users (paginated, searchable). SystemAdmin only.
 */
export async function listLdapUsers(
  params?: { offset?: number; limit?: number; search?: string },
): Promise<ListLdapUsersResponse> {
  const qs = new URLSearchParams()
  if (params?.offset != null) qs.set('offset', String(params.offset))
  if (params?.limit != null) qs.set('limit', String(params.limit))
  if (params?.search) qs.set('search', params.search)
  const suffix = qs.toString() ? `?${qs.toString()}` : ''
  const response = await get(`/api/v1/system/admin/ldap-users${suffix}`)
  // The handler returns {success:true, data:{users,total}}
  const result = response as unknown as { success: boolean; data: ListLdapUsersResponse }
  return result.data
}

/**
 * Delete an LDAP user and cascade to all related data. SystemAdmin only.
 */
export async function deleteLdapUser(userId: string): Promise<void> {
  await del(`/api/v1/system/admin/ldap-users/${encodeURIComponent(userId)}`)
}
