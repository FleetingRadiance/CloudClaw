...
export interface OrganizationMember {
  ...
  tenant_id: number
  tenant_name?: string
  user_type?: string // "ldap_user" etc., for frontend to identify LDAP users
  joined_at: string
}
...
/**
 * Transfer a tenant member from this organization to another organization (admin only).
 * Used to reassign LDAP users between admin-owned spaces.
 */
export async function transferMemberSpace(
  orgId: string,
  req: { tenant_id: number; target_org_id: string }
): Promise<ApiResponse<void>> {
  try {
    const response = await post(`/api/v1/organizations/${orgId}/transfer-member`, req)
    return response as unknown as ApiResponse<void>
  } catch (error: any) {
    return { success: false, message: error.message || 'Failed to transfer member' }
  }
}
