...
// LDAP 登录响应复用 LoginResponse
export type LdapLoginResponse = LoginResponse
...
export interface UserInfo {
  ...
  is_system_admin?: boolean
  user_type?: string  // 'admin' | 'ldap_user'
...
export function userInfoFromApi(
  ...
  return {
    ...
    ldap_login_name: u?.ldap_login_name,
 ...
 * LDAP 用户登录
 */
export async function ldapLogin(data: LdapLoginRequest): Promise<LdapLoginResponse> {
  try {
    const response = await post('/api/v1/auth/ldap-login', data)
    return response as unknown as LdapLoginResponse
  } catch (error: any) {
    return {
      success: false,
      message: error.message || t('error.auth.loginFailed')
