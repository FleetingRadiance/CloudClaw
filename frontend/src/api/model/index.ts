...
export interface InitializeCloudClowCloudRequest {
  app_id: string
  app_secret: string
}

// 仅保存 CloudClowCloud 凭证，不自动创建模型
export function saveCloudClowCloudCredentials(data: InitializeCloudClowCloudRequest): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve, reject) => {
    post('/api/v1/cloudclowcloud/credentials', data)
      .then((response: any) => {
        if (response.success) {
          resolve(response)
        } else {
      ...
      .catch((error: any) => {
        console.error('Failed to save CloudClowCloud credentials:', error)
        reject(error)
      })
  })
}

export interface CloudClowCloudStatusResult {
  has_models: boolean
  needs_reinit: boolean
  reason?: string
}

export function getCloudClowCloudStatus(): Promise<CloudClowCloudStatusResult> {
  return new Promise((resolve, reject) => {
    get('/api/v1/models/cloudclowcloud/status')
      .then((response: any) => {
