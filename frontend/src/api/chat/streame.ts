...
export function useStream() {
  ...
  const startStream = async (params: { session_id: any; query: any; knowledge_base_ids?: string[]; knowledge_ids?: string[]; agent_enabled?: boolean; agent_id?: string; web_search_enabled?: boolean; enable_memory?: boolean; summary_model_id?: string; mcp_service_ids?: string[]; mentioned_items?: Array<{id: string; name: string; type: string; kb_type?: string}>; images?: Array<{data: string}>; attachment_uploads?: Array<{data: string; file_name: string; file_size: number}>; selected_skills?: string[]; method: string; url: string }) => {
    ...
    // 获取JWT Token
    const token = localStorage.getItem('cloudclow_token');
    if (!token) {
    ...
    // defaultTenantId 时不附" 来减少 header 体积，但任何把 cloudclow_tenant
    ...
    const selectedTenantId = localStorage.getItem('cloudclow_selected_tenant_id');
    ...
    try {
      ...
      // Include selected_skills if provided (for ReAct agent mode via # button)
      if (params.selected_skills !== undefined && params.selected_skills.length > 0) {
        postBody.selected_skills = params.selected_skills;
      }
      ...
      await fetchEventSource(url, {
        ...
        onerror: (err) => {
          // 用户点击"停止生成"时调用 stopStream() → controller.abort() 会
          // 触发 AbortError。这是有意行为，不要包装成"流失败"错误，
          // 原样抛出以便外层 catch 据此跳过错误 toast。
      ...
      // AbortError 由用户主动点击停止触发（见 onerror 与 stopStream），
      // 不是真正的错误，不应弹出错误 toast。
