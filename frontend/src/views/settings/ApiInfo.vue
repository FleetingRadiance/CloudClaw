...
/** CloudClow Lite (Wails): real API origin is loopback + dynamic port, not window.location.origin */
...
type CloudClowDesktopWindow = Window & {
  __CLOUDCLOW_API_BASE__?: string
  __CLOUDCLOW_API_LAN_BASE__?: string
  go?: {
    main?: {
      App?: {
...
async function tryLoadWailsApiBaseURL() {
  const win = window as CloudClowDesktopWindow
  for (let i = 0; i < 40; i++) {
    const injected = win.__CLOUDCLOW_API_BASE__
...
async function tryLoadWailsLanHints(win: CloudClowDesktopWindow) {
  const injectedLan = win.__CLOUDCLOW_API_LAN_BASE__
...
function desktopPortBindingsAvailable(win: CloudClowDesktopWindow) {
...
function desktopBindPublicBindingsAvailable(win: CloudClowDesktopWindow) {
  const app = win.go?.main?.App
  return (
...
const openApiDoc = () => {
  window.open('https://github.com/Tencent/CloudClow/blob/main/docs/api/README.md', '_blank')
}
