                    ...
                    <div class="settings-container">
                                ...
                                <template v-for="group in navGroups" :key="group.key">
                                    ...
                                    <template v-for="item in group.items" :key="item.key">
                                        ...
                                        >
                                            ...
                                            <!-- CloudClow Cloud 使用自定义 W 图标 -->
                                            <svg
                                                v-else-if="item.key === 'cloudclowcloud'"
                                                width="17"
                                                height="17"
                                                viewBox="0 0 18 18"
                                                fill="none"
                                            ...
                                            <t-icon
                                                ...
                                                :name="
                                                    expandedMenus.includes(item.key) ? 'chevron-down' : 'chevron-right'
                                                "
                                                class="expand-icon"
                                            />
                                        </div>
                     ...
                     的瞬间触发；这一段做兜底兼容旧 URL。 -->
                                ...
                                <template v-else>
                                    ...
                                    <!-- CloudClow Cloud -->
                                    <div v-if="currentSection === 'cloudclowcloud'" class="section">
                                        <CloudClowCloudSettings />
                                    </div>
                     ...
                     用户的基本信息不该跟 owner 权限绑定。 -->
                                    ...
                                    <!-- LDAP 用户管理 -->
                                    <div v-if="currentSection === 'ldap-users'" class="section">
                                        <LdapUserManagement />
                                    </div>
                                </template>
                            </div>
...
import CloudClowCloudSettings from './CloudClowCloudSettings.vue';
import TenantMembers from './TenantMembers.vue';
import SystemSettings from '@/views/system/SystemSettings.vue';
import LdapUserManagement from './LdapUserManagement.vue';
...
const SYSTEM_ADMIN_SECTIONS = new Set(['system-global', 'ldap-users']);
...
const navItems = computed(() => {
    ...
    const all: NavItem[] = [
        ...
        { key: 'cloudclowcloud', icon: '', label: 'CloudClow Cloud' },
        ...
        { key: 'ldap-users', icon: 'user-list', label: t('ldapUserManagement.title') },
...
const navGroups = computed<NavGroup[]>(() => {
    ...
    return [
        ...
        },
        {
            ...
            items: pickItems(['models', 'ollama', 'cloudclowcloud'])
        },
        {
            key: 'data_extensions',
        ...
        },
        {
            ...
            items: pickItems(['system-global', 'ldap-users', 'system'])
        }
                ...
                if (uiStore.settingsInitialSubSection) {
                    setTimeout(() => {
                        const element = document.querySelector(
                            `[data-model-type="${uiStore.settingsInitialSubSection}"]`
                        );
                        if (element) {
                            element.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        }
