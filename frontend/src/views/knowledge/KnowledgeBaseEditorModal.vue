                    ...
                    <div class="settings-container">
                        ...
                        <div class="settings-content">
                            <div class="content-wrapper">
                                        ...
                                        <div class="section-body">
                                            <div v-if="mode === 'edit' && props.kbId" class="form-item">
                                                ...
                                                <div class="kb-id-field">
                                                    <code class="kb-id-value" :title="props.kbId">{{
                                                        props.kbId
                                                    }}</code>
                                            ...
                                            <div v-if="!isFAQ" class="form-item">
                                                ...
                                                <div
                                                    ...
                                                    :class="{ 'is-locked': isIndexingLocked || isLdapUser }"
                                                ...
                                                <p v-if="isIndexingLocked || isLdapUser" class="form-tip locked-tip">
                                                    {{ isLdapUser ? '' : $t('knowledgeEditor.indexing.lockedTip') }}
                                                </p>
                                            </div>
                                ...
                                <div v-show="currentSection === 'models'" class="section">
                                    <KBModelConfig
                                        ...
                                        :rag-enabled="
                                            ...
                                            formData.indexingStrategy?.keywordEnabled
                                        "
                                            ...
                                            >
                                                ...
                                                <div class="setting-control">
                                                    <ModelSelector
                                                        ...
                                                        :disabled="isLdapUser"
                                                        :hide-add-model="isLdapUser"
                                                        :placeholder="
                                                            $t('knowledgeEditor.advanced.multimodal.vllmPlaceholder')
                                                        "
                                                    />
                                                </div>
                                            </div>
                                ...
                                <!-- 音频处理（ASR）设置 -->
                                <div v-if="!isFAQ && !isLdapUser" v-show="currentSection === 'asr'" class="section">
                                    <div v-if="formData" class="kb-multimodal-settings">
                                        <div class="section-header">
...
const navItems = computed(() => {
    ...
    // LDAP 用户仅展示基本信息、模型配置、图像处理（可查看不可修改）、分块设置、共享管理
    if (isLdapUser.value) {
        const allowed = new Set(['basic', 'models', 'multimodal', 'chunking', 'share']);
...
const applyDefaultModelsIfEmpty = () => {
    ...
    // Knowledge base LLM: prefer the dedicated KB model over the generic default
    const kbLlm = allModels.value.find((m) => m.id === 'builtin-llm-kb-default');
    const chat = kbLlm || pick('KnowledgeQA');
    ...
    // Wiki synthesis model defaults to the same KB LLM when Wiki indexing is enabled
    if (!formData.value.modelConfig.wikiSynthesisModelId && chat?.id && formData.value.indexingStrategy?.wikiEnabled) {
        formData.value.modelConfig.wikiSynthesisModelId = chat.id;
    }
};
...
const loadKBData = async () => {
    ...
    try {
        ...
        // LDAP 用户编辑模式下强制使用知识库专用默认模型与默认分块策略
        if (isLdapUser.value) {
            const pick = (type: ModelConfig['type']) => {
                const list = allModels.value.filter((m) => m.type === type);
                return list.find((m) => m.is_default) || list[0];
            ...
            // Wiki 合成模型默认回退到当前 KB LLM
            if (!formData.value.modelConfig.wikiSynthesisModelId && chat?.id) {
                formData.value.modelConfig.wikiSynthesisModelId = chat.id;
            }
            // 分块策略为空时默认选择「自动」
            if (!formData.value.chunkingConfig.strategy) {
                formData.value.chunkingConfig.strategy = 'auto';
            }
        }
    } catch (error) {
...
const validateForm = (): boolean => {
    ...
    // 验证多模态配置（如果启用）。LDAP 用户使用系统默认视觉模型（model id 为空），
    // 属正常情况，跳过该校验。
    if (formData.value.multimodalConfig.enabled && !formData.value.multimodalConfig.vllmModelId && !isLdapUser.value) {
