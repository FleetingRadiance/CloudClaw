...
// LDAP 用户（自己个人知识库的所有者）可操作文档、查看配置，
// 但图像处理（多模态）菜单为只读、且不可移动文档。
...
const canManage = computed(() => {
  if (authStore.user?.user_type === 'ldap_user') return false;
...
const VIEW_MODE_KEY = 'cloudclow.kb.docs.viewMode';
...
onMounted(() => {
  ...
  window.addEventListener('cloudclow:open-knowledge', handleOpenKnowledgeEvent as EventListener);
});
...
onUnmounted(() => {
  ...
  window.removeEventListener('cloudclow:open-knowledge', handleOpenKnowledgeEvent as EventListener);
  stopMovePoll();
  if (timeout !== null) {
        ...
        <div class="document-header-title">
          <div class="document-title-row">
            ...
            <div class="kb-title-actions">
              ...
              <t-tooltip v-if="canManage || isLdapUser" :content="$t('knowledgeBase.settings')" placement="top">
              ...
              <div class="doc-scroll-container" :class="{ 'is-empty': !cardList.length && !docListLoading, 'is-marquee-active': docMarqueeVisible }" ref="knowledgeScroll"
                          ...
                          <t-popup v-if="canEdit" v-model="item.isMore" overlayClassName="card-more"
                            ...
                            <template #content>
                              ...
                              <div v-if="moveMenuMode === 'normal'" class="card-menu">
                                ...
                                <div v-if="canMutateKnowledge && !isLdapUser" class="card-menu-item"
                ...
                <template v-else-if="cardList.length && viewMode === 'list'">
                  <DocumentListView :items="cardList" :selected-ids="selectedIds" :tag-list="tagList"
                    :can-edit="canEdit" :can-move="canMutateKnowledge && !isLdapUser" @open="(item: any) => { if (!shouldSuppressDocClick()) openCardDetails(item); }" @toggle-row="toggleSelectRow"
