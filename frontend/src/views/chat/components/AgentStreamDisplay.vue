    ...
    <div ref="rootElement" class="agent-stream-display">
        ...
        <div v-if="shouldShowCollapsedSteps" class="tree-container">
            ...
            <div v-if="showIntermediateSteps" class="tree-children">
                <template v-for="(event, index) in displayedIntermediateEvents" :key="getEventKey(event, index)">
                    <div
                        v-if="event && event.type"
                        class="tree-child"
                        :class="{ 'tree-child-last': index === intermediateEvents.length - 1 }"
                    >
                   ...
                   reasoning is the expandable body. -->
                                ...
                                >
                                    <div class="action-header" @click="toggleEvent(event.event_id)">
                                        <div class="action-title">
                                            ...
                                            <span v-if="event.title" class="action-name action-preamble-title">{{
                                                event.title
                                            }}</span>
                                            <span v-else-if="isEventExpanded(event.event_id)" class="action-name">{{
                                                $t('agent.think')
                                            }}</span>
                                            <span v-else-if="getThinkingSummary(event)" class="action-summary">{{
                                                getThinkingSummary(event)
                                            }}</span>
                                        </div>
                                        <div v-if="event.content" class="action-show-icon">
                                            <t-icon
                                                :name="isEventExpanded(event.event_id) ? 'chevron-up' : 'chevron-down'"
                                            />
                                        </div>
                                    </div>
                            ...
                            <!-- Thinking Tool Call -->
                            <div
                                v-else-if="event.type === 'tool_call' && event.tool_name === 'thinking'"
                                class="tool-event"
                            >
                                <div
                                    class="action-card"
                                ...
                                >
                                    <div class="action-header" @click="toggleEvent(event.tool_call_id)">
                                        <div class="action-title">
                                            ...
                                            >
                                            <span
                                                v-if="getThinkingSummary(event) && !isEventExpanded(event.tool_call_id)"
                                        ...
                                        <div v-if="event.tool_data?.thought" class="action-show-icon">
                                            <t-icon
                                                :name="
                                                    isEventExpanded(event.tool_call_id) ? 'chevron-up' : 'chevron-down'
                                                "
                                            />
                                        </div>
                                    </div>
                                    <div
                                        v-if="event.tool_data?.thought && isEventExpanded(event.tool_call_id)"
                            ...
                            <!-- Tool Group (merged consecutive same-tool calls) -->
                            <div v-else-if="event.type === 'tool_group'" class="tool-event">
                                <div class="action-card">
                                    <div class="action-header" @click="toggleGroupExpand(event.group_id)">
                                        <div class="action-title">
                                            <img class="action-title-icon" :src="getToolIcon(event.tool_name)" alt="" />
                                            <span class="action-name">{{ getToolDisplayName(event.tool_name) }}</span>
                                            <span class="action-badge">{{ event.count }} 步</span>
                                        </div>
                                        <div class="action-show-icon">
                                            <t-icon
                                                :name="isGroupExpanded(event.group_id) ? 'chevron-up' : 'chevron-down'"
                                            />
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div v-else-if="event.type === 'tool_call'" class="tool-event">
                                ...
                                >
                                    ...
                                    >
                                        <div class="action-title">
                                            <img
                                                ...
                                                :src="getToolIcon(event.tool_name)"
                                                alt=""
                                            />
                                            <t-icon
                                                v-if="event.tool_name && isBookIcon(event.tool_name)"
                                            ...
                                            />
                                            <t-tooltip
                                                v-if="event.tool_name === 'todo_write' && event.tool_data?.steps"
                                    ...
                                    <div
                                        v-if="
                                            !event.pending && event.tool_name === 'todo_write' && event.tool_data?.steps
                                        "
                                    ...
                                    <div
                                        v-if="!event.pending && event.tool_name === 'web_search' && event.tool_data"
                                        class="search-results-summary-fixed"
                                    >
                                        <div
                                            class="results-summary-text"
                                            v-html="
                                                t('agent.webSearchFound', { count: getResultsCount(event.tool_data) })
                                            "
                                        ></div>
                                    </div>

                                    <div
                                        v-if="!event.pending && event.tool_name === 'grep_chunks' && event.tool_data"
                                    ...
                                    <div
                                        v-if="
                                            isEventExpanded(event.tool_call_id) && !event.pending && hasResults(event)
                                        "
                                        class="action-details"
                                    >
        ...
        <div
            ...
            :class="{ 'streaming-steps-constrained': !answerEverStarted && !isConversationDone }"
        >
            <template v-for="(event, index) in displayedInlineEvents" :key="getEventKey(event, index)">
             ...
             the expandable body. -->
                    ...
                    <div v-else-if="event.type === 'tool_group'" class="tool-event">
                        <div class="action-card">
                            <div class="action-header" @click="toggleGroupExpand(event.group_id)">
                                ...
                                <div class="action-show-icon">
                                    <t-icon :name="isGroupExpanded(event.group_id) ? 'chevron-up' : 'chevron-down'" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-else-if="event.type === 'tool_call' && event.tool_name === 'thinking'" class="tool-event">
                        ...
                        >
                            <div class="action-header" @click="toggleEvent(event.tool_call_id)">
                                ...
                                <div v-if="event.tool_data?.thought" class="action-show-icon">
                                    <t-icon
                                        :name="isEventExpanded(event.tool_call_id) ? 'chevron-up' : 'chevron-down'"
                                    />
                                </div>
                            </div>
                            <div
                    ...
                    <!-- Answer Event -->
                    <div
                        v-else-if="event.type === 'answer' && (event.done || (event.content && event.content.trim()))"
                        class="answer-event"
                    >
                        ...
                        <!-- Structured display outputs (opt-in via display_type) -->
                        <div
                            v-if="event.done && Object.keys(toolResultsByDisplayType).length > 0"
                            class="answer-generated-docs"
                        >
                            <template v-for="(items, displayType) in toolResultsByDisplayType" :key="displayType">
                                <component
                                    :is="structuredRenderers[displayType] || StructuredResultFallback"
                    ...
                    <div v-else-if="event.type === 'tool_call'" class="tool-event">
                        ...
                        >
                            ...
                            <div
                                v-if="!event.pending && event.tool_name === 'todo_write' && event.tool_data?.steps"
                                class="plan-status-summary-fixed"
                            >
                                <div class="plan-status-text">
                                    <template v-for="(part, partIndex) in getPlanStatusItems(event)" :key="partIndex">
                                        <t-icon :name="part.icon" :class="['status-icon', part.class]" />
                                        <span>{{ part.label }} {{ part.count }}</span>
                                        <span v-if="partIndex < getPlanStatusItems(event).length - 1" class="separator"
                                            >·</span
                                        >
                                    </template>
                                </div>
                            ...
                            <div
                                v-if="isEventExpanded(event.tool_call_id) && !event.pending && hasResults(event)"
                                class="action-details"
                            >
                                <div v-if="event.display_type && event.tool_data" class="tool-result-wrapper">
                                    <ToolResultRenderer
    ...
    <Teleport to="body">
        ...
        >
            <div class="t-popup__content">
                ...
                <template v-else>
                    <div v-if="floatPopup.knowledgeTitle" class="tip-meta">
                        <strong>{{ floatPopup.knowledgeTitle }}</strong>
                    </div>
                    ...
                    <div v-if="floatPopup.chunkId" class="tip-meta">
                        {{ $t('chat.chunkIdLabel') }} {{ floatPopup.chunkId }}
                    </div>
                </template>
            </div>
        </div>
            ...
            >
                <div style="display: flex; align-items: center; gap: 12px">
                    ...
                    <span class="wiki-reader-meta-text">{{
                        $t('knowledgeEditor.wikiBrowser.version', { ver: wikiDrawerPage.version || 1 })
                    }}</span>
                </div>
...
import DocumentPreviewResult from './tool-results/DocumentPreviewResult.vue';
...
import { createMermaidCodeRenderer, ensureMermaidInitialized, renderMermaidInContainer } from '@/utils/mermaidShared';
...
const DOMPurifyConfig = {
    ...
    ALLOWED_URI_REGEXP:
        /^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp):|(?:local|minio|cos|tos):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i
};
...
const TOOL_NAME_KEYS: Record<string, string> = {
    ...
    execute_data_query: 'agentStream.tools.executeDataQuery'
};
...
// Track which tool groups are expanded (for merging consecutive MCP officecli calls)
...
watch(
    ...
    (stream) => {
        ...
        for (let i = stream.length - 1; i >= 0; i--) {
            ...
            const isThinking =
                event.type === 'thinking' || (event.type === 'tool_call' && event.tool_name === 'thinking');
...
// Check if any tool call is still pending (executing or waiting for approval).
// The conversation must stay "live" while tools are running so the user can
// still stop the agent and the UI does not collapse the steps prematurely.
const hasPendingToolCalls = computed(() => {
    const stream = eventStream.value;
    if (!stream || !Array.isArray(stream)) return false;
    return stream.some(
        (e: any) =>
            (e.type === 'tool_call' && e.pending) || (e.type === 'tool_approval_required' && !e.resolved)
    );
});
...
const isConversationDone = computed(() => {
    ...
    // While tools are still running the turn is not done, even if a preamble
    // answer was already closed with done=true or a complete event arrived.
    if (hasPendingToolCalls.value) {
        console.log('[Collapse] Pending tool calls detected, conversation not done');
        return false;
    }
...
// Fallback renderer for display_type values not registered in
// structuredRenderers. Shows a compact header with the raw display_type and a
// collapsible JSON view, so users at least see the structured payload instead
// of nothing — and developers can spot unregistered types quickly.
// Note: display_type is read from toolData (not a separate prop) so that
// registered renderers like DocumentPreviewResult don't receive an
// unrecognized `display-type` attribute that would fall through to their
// root DOM element.
const StructuredResultFallback = defineComponent({
    name: 'StructuredResultFallback',
    props: {
        toolData: { type: Object as () => Record<string, any>, required: true }
    },
    setup(props) {
        ...
        return () =>
            h('div', { class: 'structured-fallback' }, [
                h('div', { class: 'fallback-header', onClick: toggle }, [
                    h('span', { class: 'fallback-icon' }, '📋'),
                    h('span', { class: 'fallback-title' }, `工具结果（${props.toolData.display_type || '未知'}）`),
                    h('span', { class: 'fallback-toggle' }, expanded.value ? '收起' : '展开')
                ]),
                expanded.value ? h('pre', { class: 'fallback-json' }, JSON.stringify(props.toolData, null, 2)) : null
            ]);
    }
});

// Collect structured display outputs (opt-in via display_type field) from the
// event stream, grouped by display_type value. The frontend routes rendering
// per group via structuredRenderers below; unrecognized display_types fall
// back to StructuredResultFallback (which shows the raw JSON in a collapsed
// panel). This keeps the rendering layer decoupled from the backend — new
// display_type values require only a new entry in structuredRenderers, with
// no backend changes.
const structuredRenderers: Record<string, any> = {
    document_preview: DocumentPreviewResult
    // Register additional display_type renderers here as new MCP servers are
    // added (e.g. pdf_preview, image_gallery, chart_preview).
};

const toolResultsByDisplayType = computed<Record<string, Array<Record<string, any>>>>(() => {
    const stream = eventStream.value;
    if (!stream || stream.length === 0) return {};
    const map: Record<string, Array<Record<string, any>>> = {};
    // Track seen file_names per display_type to deduplicate (e.g. create_document
    // may be called in multiple steps returning the same file).
    const seen = new Set<string>();
    for (const e of stream) {
        const dt = e?.tool_data?.display_type;
        if (e.type === 'tool_call' && e.success && typeof dt === 'string' && dt && structuredRenderers[dt]) {
            const fileKey = e.tool_data?.file_name;
            const dedupKey = fileKey ? `${dt}:${fileKey}` : `${dt}:${(map[dt]?.length ?? 0)}`;
            if (seen.has(dedupKey)) continue;
            seen.add(dedupKey);
            (map[dt] ||= []).push(e.tool_data);
        }
    }
    return map;
});
...
const hasDoneAnswerContent = computed(() => {
    ...
    return stream.some((e: any) => e.type === 'answer' && e.done && e.content && e.content.trim());
});
    ...
    if (answerContent.trim()) {
        ...
        for (const e of merged) {
            ...
            const titleOnlyMatches =
                e.title && !(e.content && e.content.trim()) && thinkingEqualsAnswer(e.title, answerContent);
...
// ── Tool Group Merging ─────────────────────────────────────────────────────
// Consecutive thinking + tool_call events with the SAME tool_name are merged
// into a single collapsible "tool_group" card. This is generic — works for
// any tool (officecli, web_search, knowledge_search, etc.) that is called
// multiple times in a row. Different tool names break the group.
const isGroupableToolCall = (event: any): boolean => {
    return event.type === 'tool_call' && typeof event.tool_name === 'string' && event.tool_name !== 'thinking';
};

// Generate a human-readable display name from a tool_name
const getToolDisplayName = (toolName: string): string => {
    ...
    if (knownNames[toolName]) return knownNames[toolName];

    // MCP tools: mcp_{service}_{tool} → "{service} - {tool}"
    if (toolName.startsWith('mcp_')) {
...
// Reusable grouping function — works on any event array (intermediate or inline).
// Groups consecutive thinking + same-tool tool_call events into tool_group cards.
// 归并卡片只做视觉折叠：显示工具名 + 步数；状态由各子卡片自行展示。
const groupConsecutiveToolCalls = (events: any[]): any[] => {
    ...
    const flushGroup = () => {
        ...
        if (currentGroup.length <= 1) {
            // Single event — no need to group
            result.push(...currentGroup);
        } else {
            const toolCalls = currentGroup.filter((e: any) => e.type === 'tool_call');
            const toolName = currentToolName || toolCalls[0]?.tool_name || 'unknown';
            // 用首个 tool_call_id 生成稳定 group_id，避免事件流重算时展开状态丢失
            const firstCallId = toolCalls[0]?.tool_call_id || `idx-${result.length}`;
            const groupId = `tool-group-${firstCallId}`;
            ...
            result.push({
                ...
                count: toolCalls.length, // 步数 = 工具调用数（不含 thinking）
    ...
    for (const event of events) {
        if (isGroupableToolCall(event)) {
            if (currentToolName === null) {
                // Start new group
                currentToolName = event.tool_name;
                currentGroup.push(event);
            } else if (event.tool_name === currentToolName) {
                // Same tool — add to current group, step count +1
                currentGroup.push(event);
            } else {
                // Different tool — flush and start new group
                flushGroup();
                currentToolName = event.tool_name;
                currentGroup.push(event);
            }
        } else if (event.type === 'thinking' && currentGroup.length > 0) {
            // Thinking between same-tool calls — include in group
            currentGroup.push(event);
        } else {
            // Other events break the group
            flushGroup();
            result.push(event);
        }
    }
...
// Expand groups into their children for display. Collapsed groups show as
// a single card; expanded groups show the header plus all children inline.
const expandGroupedEvents = (grouped: any[]): any[] => {
    const result: any[] = [];
    for (const event of grouped) {
        if (event.type === 'tool_group') {
            result.push(event);
            if (isGroupExpanded(event.group_id)) {
...
const groupedIntermediateEvents = computed(() => groupConsecutiveToolCalls(intermediateEvents.value));
...
// Tree display: grouped + expanded
const displayedIntermediateEvents = computed(() => expandGroupedEvents(groupedIntermediateEvents.value));

// Inline display: also apply grouping (while conversation is running)
const displayedInlineEvents = computed(() => expandGroupedEvents(groupConsecutiveToolCalls(displayEvents.value)));
...
const displayEvents = computed(() => {
    ...
    if (final.type === 'thinking') {
        ...
        const thinking = result.find((e: any) => e.type === 'thinking' && e.event_id === final.event_id);
...
const preprocessMarkdown = (contentStr: string): string => {
    ...
    return contentStr
        .replace(/<web\b([^>]*)\/>/g, (_m: string, attrString: string) => {
...
const HTML_PLACEHOLDER_RE = /@@CLOUDCLOW_HTML_PLACEHOLDER_(\d+)@@/g;
...
const extractRenderableHtmlPlaceholders = (contentStr: string): { content: string; htmlSnippets: string[] } => {
    ...
    const storeHtml = (html: string): string => {
        ...
        return `@@CLOUDCLOW_HTML_PLACEHOLDER_${idx}@@`;
    };
...
const getGrepResultsSummary = (toolData: any): string => {
    ...
    const rowCount = toolData.chunk_results?.length ?? toolData.result_count ?? toolData.knowledge_results?.length ?? 0;
...
const getQueryText = (args: any): string => {
    ...
    if (Array.isArray(parsedArgs.queries) && parsedArgs.queries.length > 0) {
        queries.push(...parsedArgs.queries.filter((q: any) => q && typeof q === 'string'));
    }
...
.answer-event {
    ...
    .answer-generated-docs {
        margin-top: 8px;
    }

    /* Fallback renderer for unrecognized display_type values */
    .structured-fallback {
        margin: 8px 0;
