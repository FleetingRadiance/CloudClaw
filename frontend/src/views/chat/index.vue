        ...
        <div class="chat" :class="{ 'is-embedded': embeddedMode, 'is-sidebar-collapsed': uiStore.sidebarCollapsed }">
            <div ref="scrollContainer" class="chat_scroll_box" @scroll="handleScroll">
                    ...
                    >
                        ...
                        >
                            <div class="suggested-questions-title">
                                <t-skeleton animation="gradient" :row-col="[{ width: '120px', height: '18px' }]" />
                            </div>
                            ...
                            <div v-if="suggestedQuestions.length > 0" class="suggested-questions-inner">
                                ...
                                <div class="suggested-questions-grid">
                                    ...
                                    >
                                        ...
                                        <span v-if="item.source === 'faq'" class="suggested-question-badge faq"
                                            >FAQ</span
                                        >
                                    </div>
                                </div>
                ...
                -->
                    ...
                    <div v-if="loading" style="height: 41px; display: flex; align-items: center; padding-left: 4px">
...
watch(() => useSettingsStoreInstance.settings.selectedKnowledgeBases, debouncedFetchSuggestions, { deep: true });
watch(() => useSettingsStoreInstance.settings.selectedFiles, debouncedFetchSuggestions, { deep: true });
...
const reconstructEventStreamFromSteps = (
    ...
    if (agentSteps && Array.isArray(agentSteps) && agentSteps.length > 0) {
        agentSteps.forEach((step) => {
            ...
            const reasoningText = step.reasoning_content && step.reasoning_content.trim() ? step.reasoning_content : '';
            if (reasoningText) {
                events.push({
                    type: 'thinking',
...
// 发送消息
// 处理停止生成事件 - 立即清除 loading 状态并 abort 旧 SSE 连接
//
// 此前仅清前端状态而不调 stopStream()，会导致旧 fetchEventSource 继续
// 接收后端事件并通过共享 chunkHandler 流入下一条对话，造成"暂停后旧
// 思考-工具链在新对话后执行"的混乱。立即 abort 后旧的 SSE 不再喂事
// 件给前端；stopping 中间态用于在 stopSession API 真正完成后端取消前
// 禁用发送按钮，避免用户抢发触发竞态。
const stopping = ref(false);
const handleStopGeneration = () => {
    console.log('[Stop Generation] Immediately clearing loading state and aborting SSE');
    stopStream();
    loading.value = false;
    isReplying.value = false;
    stopping.value = true;
    // 立即标记当前不完整的 assistant 消息为已完成，
    // 防止后续 findLastMessage() 将其匹配并追加新内容导致位置错乱
    const incompleteMsg = findLastMessage((item) => item.role === 'assistant' && !item.is_completed);
    if (incompleteMsg) {
        incompleteMsg.is_completed = true;
        console.log('[Stop Generation] Marked incomplete assistant message as completed');
    }
    currentAssistantMessageId.value = '';
};

// stopSession API（在 Input-field.vue 中发起）结束后回调：
// 解除 stopping 状态，恢复发送按钮可用
...
const sendMsg = async (value, modelId = '', mentionedItems = [], imageFiles = [], attachmentFiles = []) => {
    ...
    // Get selected skills from settings store (for ReAct agent mode via # button)
    const selectedSkills = props.embeddedMode ? [] : useSettingsStoreInstance.settings.selectedSkills || [];
...
watch(error, (newError) => {
    if (newError) {
        ...
        loading.value = false;
        // 注意：不在流错误时立即设置 isReplying=false。
        // 如果 agent 会话中有未完成的工具调用（如技能执行失败导致流中断），
        // 保持停止按钮可用，让用户可以尝试停止后端仍在运行的 agent。
        const lastMsg = findLastMessage((item) => item.role === 'assistant');
        const hasPendingTools = lastMsg?._pendingToolCalls?.size > 0 ||
            lastMsg?.agentEventStream?.some((e) => e.type === 'tool_call' && e.pending);
        if (!hasPendingTools) {
            isReplying.value = false;
            currentAssistantMessageId.value = '';
        } else {
            console.warn('[Stream Error] Stream error but仍有 pending 工具调用, keeping stop button available');
        }
    }
});
...
onChunk((data) => {
    ...
    const isAgentAnswerChunk = data.response_type === 'answer' && (isAgentStreamSession() || targetsActiveAgentRequest);
    ...
    const shouldHandleAsAgent =
        isAgentOnlyResponse || isCurrentlyAgentMode || isAgentAnswerChunk || isAgentCompleteChunk;
    ...
    switch (data.response_type) {
        ...
        case 'tool_call':
            ...
            if (data.data && (data.data.tool_name || data.data.tool_call_id)) {
                ...
                const toolCallId =
                    data.data.tool_call_id || (incomingToolName ? incomingToolName + '_' + Date.now() : null);
                if (!toolCallId) {
                ...
                // 只要有新的工具调用开始，说明 Agent 仍在运行。之前 answer.done 可能
                // 因尚未收到 tool_call 而错误地关闭了 isReplying，这里重新打开停止按钮。
                if (!isReplying.value) {
                    isReplying.value = true;
                    console.log('[Tool Call] Resuming isReplying because a new tool call started');
                }
            }
            break;
        ...
        case 'answer': {
            ...
            if (data.done && !answerEvent.done) {
                ...
                // 检查是否仍有 pending 的工具调用（如技能执行失败后端过早发送 done）
                const hasPendingTools = message._pendingToolCalls?.size > 0 ||
                    message.agentEventStream?.some((e) => e.type === 'tool_call' && e.pending);
                ...
                if (!hasPendingTools) {
                    ...
                    console.warn('[Answer Done] Still have pending tool calls, keeping stop button available');
                }
            }
            break;
        }
        ...
        case 'complete': {
            ...
            // 检查是否仍有 pending 的工具调用或未完成的工具事件。
            // 后端可能在技能失败后过早发送 complete，导致前端停止按钮消失
            // 而后端 agent 仍在执行。此时保持 isReplying=true 以保留停止能力。
            const hasPendingTools = message._pendingToolCalls?.size > 0 ||
                message.agentEventStream?.some((e) =>
                    (e.type === 'tool_call' && e.pending) ||
                    (e.type === 'tool_result' && e.success === false && !e._acknowledged)
                );

            // 标记已收到的 tool_error 为已处理，避免重复警告
            if (message.agentEventStream) {
                message.agentEventStream.forEach((e) => {
                    if (e.type === 'tool_result' && e.success === false) e._acknowledged = true;
                });
            }
            ...
            if (!hasPendingTools) {
                ...
                // 后端可能过早发送 complete（如技能失败后）。若仍有工具在执行，
                // 不标记消息完成，也不推 agent_complete，避免对话显示已结束但
                // 工具仍在跑、且无法暂停。
                console.warn('[Complete] Received complete but仍有 pending/failed 工具调用, keeping stop button available');
            }
            break;
        }
...
.chat {
    ...
    // 只使用 flex:1 + min-height:0 获取父容器分配的有界高度。
    // 不要同时写 height:100%：嵌套 flex 中被拉伸得到的高度在某些浏览器下
    // 不被视为 definite，height:100% 会退化为 auto，导致 .chat_scroll_box
    // 被内容撑开、overflow-y:auto 无法触发（对话过长无法滚动）。
    flex: 1;
    min-height: 0;
    // 保险：确保在有界 flex 父级下高度不会超过父容器，防止被内容撑开导致无法滚动
    max-height: 100%;
    position: relative;
    display: flex;
    flex-direction: column;
...
.chat_scroll_box {
    ...
    min-height: 0;
    // 冗余保险：确保滚动容器高度被限制在父容器内，否则内容会撑开容器导致无滚动条
    max-height: 100%;
    width: 100%;
    overflow-y: auto;
    // 阻止滚动链：在触控板/触屏上滚到列表顶/底部时，不把滚动事件冒泡到外层
    // 页面（避免移动端下拉刷新、或把整个应用跟着拉动）。
    overscroll-behavior: contain;
    // 始终为滚动条预留轨道空间（scrollbar-gutter），并让滚动条"默认就可见"：
    // 之前是 6px 极细条 + 仅 hover 才显色，用户容易误以为此处不能滚动。
    // 改为默认显示一条淡色细条、hover 加深，且预留 gutter 避免布局抖动，
    // 让"这里有可滚动区域"一眼可辨。Firefox 用 scrollbar-*，WebKit 用伪元素。
    scrollbar-gutter: stable;
    scrollbar-width: thin;
    scrollbar-color: var(--td-text-color-placeholder) transparent;
    ...
    &:hover {
        scrollbar-color: var(--td-text-color-secondary) transparent;
