...
import { BUILTIN_QUICK_ANSWER_ID, BUILTIN_SMART_ID } from "@/api/agent";
...
interface Settings {
  ...
  selectedSkills: string[];  // 当前选中的技能名称列表（ReAct智能体通过#按钮选择）
...
export const useSettingsStore = defineStore("settings", {
  state: () => ({
    ...
    settings: JSON.parse(localStorage.getItem("CloudClow_settings") || JSON.stringify(defaultSettings)) as Settings,
  ...
  actions: {
    ...
    saveSettings(settings: Settings) {
      ...
      // 保存到localStorage
      localStorage.setItem("CloudClow_settings", JSON.stringify(this.settings));
    },
    ...
    selectAgent(agentId: string, sourceTenantId?: string | null) {
      ...
      } else if (agentId === BUILTIN_SMART_ID) {
    ...
    // 添加选中的技能
    addSelectedSkill(skillName: string) {
    ...
    removeSelectedSkill(skillName: string) {
      ...
      this.settings.selectedSkills = this.settings.selectedSkills.filter((s: string) => s !== skillName);
      localStorage.setItem("CloudClow_settings", JSON.stringify(this.settings));
    },
    ...
    getSelectedSkills(): string[] {
      return this.settings.selectedSkills || [];
    },
    ...
    applyLastRequestState(state: SessionLastRequestStatePayload | null | undefined) {
      ...
      try {
        ...
        if (Array.isArray(state.selected_skills)) {
          this.settings.selectedSkills = [...state.selected_skills];
        }
      } finally {
