...
import { listSkillsForUser, listSkills, type SkillInfo } from '@/api/skill'
  ...
  async function ensureSkills(force = false): Promise<void> {
    return runOnce('skills', force, async () => {
      try {
        ...
        const skillsRes = res as any
        skillsAvailable.value = skillsRes?.skills_available !== false
        skills.value = skillsRes?.data && skillsRes.data.length > 0 ? skillsRes.data : []
        // Fallback: if the user-visible endpoint returns no skills, try the
        // preloaded-only endpoint so built-in skills still show for LDAP users.
        if (skills.value.length === 0) {
          try {
            const preloadedRes = await listSkills() as any
            const preloadedData = preloadedRes
            if (preloadedData?.data && preloadedData.data.length > 0) {
              skills.value = preloadedData.data
              skillsAvailable.value = preloadedData?.skills_available !== false
            }
          } catch {
            // keep the empty result from the primary call
          }
        }
      } catch {
