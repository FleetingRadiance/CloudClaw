...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`cloudclow_memberships`)
  ...
  const setUser = (userData: UserInfo) => {
    ...
    user.value = userData
    // 保存到localStorage
    localStorage.setItem('cloudclow_user', JSON.stringify(userData))
  ...
  const setTenant = (tenantData: TenantInfo) => {
    ...
    // 保存到localStorage
    localStorage.setItem('cloudclow_tenant', JSON.stringify(tenantData))
  }

  const setToken = (tokenValue: string) => {
    token.value = tokenValue
    localStorage.setItem('cloudclow_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('cloudclow_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('cloudclow_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('cloudclow_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('cloudclow_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('cloudclow_last_chat_model_id')
      localStorage.removeItem('cloudclow_current_kb')
      const raw = localStorage.getItem('CloudClow_settings')
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('CloudClow_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('cloudclow_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('cloudclow_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('cloudclow_selected_tenant_id')
      localStorage.removeItem('cloudclow_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('cloudclow_memberships', JSON.stringify(memberships.value))
  }
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('cloudclow_lite_mode', 'true')
    } else {
      localStorage.removeItem('cloudclow_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    // 清空localStorage
    localStorage.removeItem('cloudclow_user')
    localStorage.removeItem('cloudclow_tenant')
    localStorage.removeItem('cloudclow_token')
    localStorage.removeItem('cloudclow_refresh_token')
    localStorage.removeItem('cloudclow_knowledge_bases')
    localStorage.removeItem('cloudclow_current_kb')
    localStorage.removeItem('cloudclow_selected_tenant_id')
    localStorage.removeItem('cloudclow_selected_tenant_name')
    localStorage.removeItem('cloudclow_memberships')
    localStorage.removeItem('cloudclow_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('cloudclow_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    // 从localStorage恢复状态
    const storedUser = localStorage.getItem('cloudclow_user')
    const storedTenant = localStorage.getItem('cloudclow_tenant')
    const storedToken = localStorage.getItem('cloudclow_token')
    const storedRefreshToken = localStorage.getItem('cloudclow_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('cloudclow_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('cloudclow_current_kb')
    const storedSelectedTenantId = localStorage.getItem('cloudclow_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('cloudclow_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('cloudclow_memberships')
    ...
    isLiteMode.value = localStorage.getItem('cloudclow_lite_mode') === 'true'
  }
