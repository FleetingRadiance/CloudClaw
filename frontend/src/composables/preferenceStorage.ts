 ...
 *
 * Storage layout: CloudClow_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (CloudClow_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('cloudclow_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[CloudClow] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `CloudClow_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `CloudClow_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `CloudClow_anon_${suffix}`
    const legacyKey = `CloudClow_${suffix}`
