  ...
  <div class="user-menu" :class="{ 'user-menu--collapsed': uiStore.sidebarCollapsed }" ref="menuRef">
    ...
    <Transition name="dropdown">
      <div v-if="menuVisible" class="user-dropdown" @click.stop>
        ...
        <div v-if="userName && !authStore.isLiteMode && !props.isLdapUser" ref="tenantMenuItemRef" class="dropdown-tenant-panel" :class="{
        ...
        <template v-if="!props.isLdapUser">
...
const CLAWHUB_SKILL_URL = 'https://clawhub.ai/lyingbug/cloudclow'

// 打开 CloudClow Chrome 插件（Chrome应用商店）
...
const openGithub = () => {
  menuVisible.value = false
  window.open('https://github.com/Tencent/CloudClow', '_blank')
}
