...
import { type CustomAgent, BUILTIN_QUICK_ANSWER_ID, BUILTIN_SMART_ID, BUILTIN_WIKI_RESEARCHER_ID } from '@/api/agent';
...
const { agents, disabledOwnAgentIds, chatModels: availableModels, webSearchProviders } = storeToRefs(chatResources);
const { t } = useI18n();
...
const CHAT_FILE_DROP_EVENT = 'cloudclow:chat-file-drop';
...
const selectedAgent = computed(() => {
    ...
    // Builtin agents (e.g. builtin-smart) use the same constant ID across
...
// 判断是否为 ReAct 智能体（仅 builtin-smart，用于显示 skill 选择按钮）
const isReActAgent = computed(() => {
    return selectedAgentId.value === BUILTIN_SMART_ID;
});

// 判断是否为 builtin-smart 智能体（用于隐藏知识库按钮、显示联网按钮）
const isBuiltinSmartAgent = computed(() => {
    return selectedAgentId.value === BUILTIN_SMART_ID;
});

// 判断是否为业务咨询智能体（builtin-wiki-researcher）
const isWikiResearcherAgent = computed(() => {
    return selectedAgentId.value === BUILTIN_WIKI_RESEARCHER_ID;
});

// 技能选择器状态
const showSkillSelector = ref(false);
const skillSelectorStyle = ref<Record<string, string>>({});
...
watch(
    ...
    ([newAgentId, newAgentKbs, newKbMode], [oldAgentId]) => {
        ...
        if (newAgentId !== oldAgentId && oldAgentId !== undefined) {
            ...
            // 切换到非 ReAct 智能体时清空 skill 选择
            if (!isReActAgent.value && settingsStore.settings.selectedSkills?.length > 0) {
                settingsStore.updateSettings({ selectedSkills: [] });
            }
        }
    },
    { immediate: true }
...
// Check if current user is an LDAP normal user (should hide web search, image upload, model selector)
const isLdapUser = computed(() => authStore.user?.user_type === 'ldap_user');

// LDAP 用户仅在 ReAct 智能体（builtin-smart）时显示联网搜索按钮
const canShowWebSearchButton = computed(() => {
    if (isLdapUser.value) {
        // LDAP 用户只有选择 builtin-smart 时才显示联网按钮
        return isBuiltinSmartAgent.value;
    }
    return true;
});
...
const isWebSearchDisabledByAgent = computed(() => {
    // LDAP 用户使用 builtin-smart 时，网络搜索默认关闭但可手动开启，不禁用按钮
    if (isLdapUser.value && isBuiltinSmartAgent.value) {
        return false;
    }
...
const props = defineProps({
    ...
    // 停止中状态：stopSession API 已发出但后端尚未真正取消 agent 执行。
    // 期间禁用发送按钮，防止用户抢发新消息触发竞态（旧 SSE 已 abort，
    // 但后端 agent 可能仍在跑，新请求复用同一 EventBus 会产生事件串流）。
    stopping: {
        type: Boolean,
        default: false
    },
...
const selectedKbs = computed(() => {
    ...
    const sharedFromAgent = (sharedAgentKbList.value || [])
        .filter((kb) => selectedKbIds.value.includes(kb.id) && !ownIds.has(kb.id) && !sharedOnlyIds.has(kb.id))
        .map((kb) => ({
            id: kb.id,
            name: kb.name,
            type: kb.type || 'document',
...
const isWebSearchConfigured = computed(() => {
    ...
    if (agentProviderId) {
        // "qwen" is a system-level synthetic provider (通义千问联网搜索),
        // always available without DB configuration.
        if (agentProviderId === 'qwen') return true;
...
const ensureSelectedAgentNotDisabled = () => {
    ...
    const isEnabled = (id: string) => agents.value.some((a) => a.id === id) && !disabledOwnAgentIds.value.includes(id);

    // 所有用户的 ReAct 智能体统一为 builtin-smart
    const reactAgentId = BUILTIN_SMART_ID;
    let fallback: CustomAgent | undefined;
    if (isEnabled(reactAgentId)) {
        fallback = agents.value.find((a) => a.id === reactAgentId);
    ...
    // selectAgent 内部仅对 builtin 常量自动切 isAgentEnabled；自定义 agent 兜底时
    // 需要按其 agent_mode 显式同步一次，保证模式徽标与对话行为一致。
    if (fallback.id !== BUILTIN_QUICK_ANSWER_ID && fallback.id !== reactAgentId) {
        settingsStore.toggleAgent(fallback.config?.agent_mode === 'smart-reasoning');
    }
};

// 对话下拉中展示的「我的」智能体（排除当前租户已停用的）
const enabledAgents = computed(() => agents.value.filter((a) => !disabledOwnAgentIds.value.includes(a.id)));

// LDAP 用户可用的智能体：内置维基问答 + 管理员配置的自定义共享智能体
const ldapAvailableAgents = computed(() => {
    const result: Array<{ agent: CustomAgent; sourceTenantId?: string }> = [];

    // 1. 内置智能体：智能体（builtin-smart）
    const smartAgent = agents.value.find((a) => a.id === BUILTIN_SMART_ID);
    if (smartAgent) {
        result.push({ agent: smartAgent });
    }

    // 2. 内置维基问答
    const wikiAgent = agents.value.find((a) => a.id === BUILTIN_WIKI_RESEARCHER_ID);
    if (wikiAgent) {
        result.push({ agent: wikiAgent });
    }

    // 3. 共享的自定义智能体（非内置）
    if (orgStore.sharedAgents?.length) {
        orgStore.sharedAgents.forEach((shared: any) => {
            if (shared.agent && !shared.agent.is_builtin && !shared.disabled_by_me) {
                result.push({ agent: shared.agent as CustomAgent, sourceTenantId: String(shared.source_tenant_id) });
            }
        });
    }
...
const LAST_CHAT_MODEL_KEY = 'cloudclow_last_chat_model_id';
...
const ensureModelSelection = () => {
    ...
    if (availableModels.value.length > 0) {
        // LDAP 用户默认使用 LLM 模型
        if (isLdapUser.value) {
            const llmDefault = availableModels.value.find((m) => m.id === 'builtin-llm-default');
            selectedModelId.value = llmDefault?.id || availableModels.value[0].id || '';
        } else {
...
const loadMentionItems = async (q: string, resetIndex = true, append = false) => {
    ...
    if (!append) {
        ...
        mentionAllowedKbIds.value = hasAgentConfig.value ? new Set(availableKbs.map((kb: any) => String(kb.id))) : null;
...
const toggleSkillSelector = () => {
    ...
    // 计算位置
    const btnEl = skillButtonRef.value;
    if (!btnEl) return;
    const zoom = getRootZoom();
    const rect = rectToCssPx(btnEl.getBoundingClientRect(), zoom);
    const { height: vh } = cssViewportSize(zoom);
...
const handleSkillSelectorUpdate = (skills: string[]) => {
    settingsStore.settings.selectedSkills = [...skills];
    localStorage.setItem('CloudClow_settings', JSON.stringify(settingsStore.settings));
};

const removeSelectedSkill = (skillName: string) => {
    settingsStore.removeSelectedSkill(skillName);
};
...
onMounted(() => {
    ...
    void Promise.all([loadKnowledgeBases(), loadWebSearchConfig(), loadChatModels(), loadAgents()]);
...
const emit = defineEmits<{
    ...
    // stopSession API 完成（成功或失败）后触发，父组件据此解除 stopping 状态
    (e: 'stop-done'): void;
}>();
...
const createSession = async (val: string) => {
    ...
    // stopping 期间后端尚未真正取消 agent 执行，禁止抢先发新消息
    if (props.stopping) {
        return MessagePlugin.info(t('input.stopping'));
    }
...
const selectAgentMode = (mode: 'quick-answer' | 'smart-reasoning') => {
    // 所有用户的 ReAct 智能体统一为 builtin-smart
    const reactAgentId = BUILTIN_SMART_ID;
    const builtinAgentId = mode === 'smart-reasoning' ? reactAgentId : BUILTIN_QUICK_ANSWER_ID;
    ...
    if (shouldEnableAgent !== isAgentEnabled.value) {
        ...
        // 同时更新选中的智能体
        settingsStore.selectAgent(shouldEnableAgent ? BUILTIN_SMART_ID : BUILTIN_QUICK_ANSWER_ID);
        MessagePlugin.success(
            shouldEnableAgent ? t('input.messages.agentSwitchedOn') : t('input.messages.agentSwitchedOff')
        );
    }
    showAgentModeSelector.value = false;
};
...
const handleSelectAgent = (agent: CustomAgent, sourceTenantId?: string) => {
    ...
    const actualAgent = agent.is_builtin ? agents.value.find((a) => a.id === agent.id) || agent : agent;
    ...
    // 1. 同步网络搜索状态
    // 对话页面：切换到 builtin-smart 时默认关闭网络搜索（用户可手动开启）
    // 其他智能体：使用智能体配置的 web_search_enabled 值
    if (agent.id === BUILTIN_SMART_ID) {
        // ReAct 智能体：对话时默认关闭网络搜索，用户可手动开启
        settingsStore.toggleWebSearch(false);
    } else {
        ...
        } else if (isLdapUser.value) {
            // LDAP 用户切换到其他智能体时关闭联网搜索
            settingsStore.toggleWebSearch(false);
        }
    }
    ...
    const isModeBuiltin = agent.id === BUILTIN_QUICK_ANSWER_ID || agent.id === BUILTIN_SMART_ID;
...
const handleStop = async () => {
    ...
    // 发送 stop 事件，通知父组件立即清除 loading 状态并 abort SSE 连接
    emit('stop-generation');

    // 兜底超时：若 stopSession API 因后端重启/网络异常永远不返回，
    // 5 秒后强制解除 stopping 状态，避免发送按钮永久禁用
    let timeoutFired = false;
    const timeoutId = window.setTimeout(() => {
        timeoutFired = true;
        emit('stop-done');
        console.warn('[Stop] stopSession timed out after 5s, force releasing stopping state');
    }, 5000);
...
<template>
    <div class="answers-input" @drop="onDrop" @dragover="onDragOver">
        ...
        <div class="rich-input-container" data-guide="chat-input">
                ...
                >
                    <span class="mention-chip__icon-wrap" :class="{ 'has-org': item.org_name }">
                        ...
                        <span v-if="item.org_name" class="mention-chip__org-badge">
                            <img
                                :src="
                                    getImgSrc(item.type === 'file' ? 'organization-grey.svg' : 'organization-green.svg')
                                "
            ...
            <!-- 选中的技能标签（ReAct模式下显示在输入框内顶部） -->
            <div v-if="isReActAgent && settingsStore.settings.selectedSkills?.length > 0" class="selected-tags-inline">
                <span
                    v-for="skillName in settingsStore.settings.selectedSkills"
                    :key="skillName"
                ...
                >
                    ...
                    <span class="mention-chip__name" :title="skillName">{{ skillName }}</span>
                    <span
                        class="mention-chip__remove"
                        @click.stop="removeSelectedSkill(skillName)"
                        :aria-label="$t('chat.selectedSkillRemove')"
                        >×</span
                    >
                </span>
            </div>
        ...
        <!-- Skill Selector (ReAct模式) -->
        <Teleport to="body">
            <SkillSelector
                :visible="showSkillSelector"
                :style="skillSelectorStyle"
                :selectedSkills="settingsStore.settings.selectedSkills || []"
                @update:selectedSkills="handleSkillSelectorUpdate"
        ...
        <div class="control-bar">
            ...
            <div class="control-left" v-if="!embeddedMode">
                <!-- Agent 并排选择按钮（LDAP 用户） -->
                <template v-if="isLdapUser">
                    <button
                        v-for="item in ldapAvailableAgents"
                        :key="item.sourceTenantId ? `${item.agent.id}-${item.sourceTenantId}` : item.agent.id"
                        class="ldap-agent-btn"
                        :class="{
                            active:
                                selectedAgentId === item.agent.id &&
                                (item.sourceTenantId
                                    ? settingsStore.selectedAgentSourceTenantId === item.sourceTenantId
                                    : !settingsStore.selectedAgentSourceTenantId)
                        }"
                        @click="handleSelectAgent(item.agent, item.sourceTenantId)"
                    >
                        {{ item.agent.name }}
                    </button>
                </template>

                <!-- Agent 模式切换按钮（非 LDAP 用户） -->
                <template v-else>
                    <div
                ...
                <!-- WebSearch 开关按钮（LDAP 用户仅在 ReAct 智能体下显示） -->
                <t-tooltip
                    v-if="canShowWebSearchButton"
                    placement="top"
                    theme="light"
                ...
                <!-- @ 知识库/文件选择按钮（builtin-smart 智能体模式下隐藏） -->
                <t-tooltip
                    v-if="!isBuiltinSmartAgent"
                    placement="top"
                    theme="light"
                    :popupProps="{ overlayClassName: 'input-field-tooltip' }"
                >
                ...
                >
                    <template #content>
                        <span>{{
                            settingsStore.settings.selectedSkills?.length > 0
                                ? $t('chat.skillSelector.title') +
                                  ' (' +
                                  settingsStore.settings.selectedSkills.length +
                                  ')'
                                : $t('chat.skillSelector.title')
                        }}</span>
                    </template>
                    <div
                        ...
                        :class="{
                            active: settingsStore.settings.selectedSkills?.length > 0
                        }"
                        @click.stop="toggleSkillSelector"
                    >
                        <svg
                            width="18"
                            height="18"
                        ...
                        <span v-if="settingsStore.settings.selectedSkills?.length > 0" class="kb-count">{{
                            settingsStore.settings.selectedSkills.length
                        }}</span>
                    </div>
                </t-tooltip>

                <!-- 模型显示 (LDAP普通用户不显示下拉选择器) -->
                <t-tooltip
                    v-if="!isLdapUser"
                        ...
                        <div class="model-selector-content">
                            ...
                            >
                                <div class="model-option-main">
                                    ...
                                    <span v-if="model.display_name" class="model-option-raw-name">{{
                                        model.name
                                    }}</span>
                                    <span v-if="model.source === 'remote'" class="model-badge-remote">{{
                                        $t('input.remote')
                                    }}</span>
            ...
            <div class="control-right">
                <!-- 停止按钮（回复中或停止中时显示；stopping 期间禁用避免重复点击） -->
                <t-tooltip
                    v-if="isReplying || stopping"
                    :content="stopping ? $t('input.stopping') : $t('input.stopGeneration')"
                    placement="top"
                >
                    <div
                        @click="stopping ? null : handleStop()"
                        class="control-btn stop-btn"
                        :class="{ disabled: stopping }"
                    >
...
.ldap-agent-btn:hover {
    border-color: var(--td-brand-color, #0052d9);
    color: var(--td-brand-color, #0052d9);
}
.ldap-agent-btn.active {
