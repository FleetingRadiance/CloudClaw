    ...
    <div class="aside_box" :class="{ 'aside_box--collapsed': uiStore.sidebarCollapsed }">
        ...
        <div class="logo_row" v-if="!uiStore.sidebarCollapsed">
            <div class="logo_box" @click="router.push('/platform/knowledge-bases')" style="cursor: pointer">
                <img class="logo" src="@/assets/img/cloudclow.png" alt="商业西北智能体" />
        ...
        <div class="menu_top">
            ...
            >
                    ...
                    >
                        <div class="menu_item-box">
                            ...
                            <template v-if="!uiStore.sidebarCollapsed">
                                ...
                                >
                                <span
                                    v-if="item.path === 'creatChat' && batchMode"
                                    class="batch-cancel-hint"
                                    @click.stop="exitBatchMode"
                                    >{{ t('batchManage.cancel') }}</span
                                >
                    ...
                    <template v-if="loading && groupedSessions.length === 0">
                        <div v-for="n in 5" :key="'skel-' + n" class="submenu_item_p">
                            <div class="submenu_item">
                                <t-skeleton
                                    animation="gradient"
                                    style="margin-left: 14px; width: 80%"
                    ...
                    <template v-for="(group, groupIndex) in groupedSessions" :key="groupIndex">
                        ...
                        <div class="submenu_item_p" v-for="(subitem, subindex) in group.items" :key="subitem.id">
                            <div
                                :class="[
                                    'submenu_item',
                                    !batchMode && currentSecondpath == subitem.path ? 'submenu_item_active' : '',
                                    batchMode && batchSelectedIds.includes(subitem.id) ? 'submenu_item_selected' : '',
                                    batchMode ? 'submenu_item_batch' : ''
                                ]"
                                @mouseenter="mouseenteBotDownr(subitem.id)"
                            ...
                            >
                                ...
                                >
                                    <t-icon
                                        v-if="subitem.is_pinned"
                                        name="pin"
                                        class="submenu_pin_icon"
                                        :title="t('menu.pinned')"
                                    />
                                    <img
                                        v-if="subitem.im_platform && platformLogo(subitem.im_platform)"
                ...
                <div
                    v-if="batchMode && item.path === 'creatChat' && !uiStore.sidebarCollapsed"
        ...
        <div class="menu_bottom">
            <UserMenu :is-ldap-user="isLdapUser" />
        </div>
    </div>
</template>
...
const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        ...
        // LDAP 用户不展示设置菜单
        if (item.path === 'settings' && isLdapUser.value) {
            return false;
        }
        return true;
    });
...
const getMessageList = async (isLoadMore = false) => {
    ...
    return getSessionsList(currentPage.value, page_size.value)
        .then((res: any) => {
...
watch([() => route.name, () => route.params], (newvalue, oldvalue) => {
    ...
    const oldRouteNameStr =
        typeof oldvalue?.[0] === 'string' ? (oldvalue[0] as string) : oldvalue?.[0] ? String(oldvalue[0]) : '';
    const leavingCreatChat =
        (oldRouteNameStr === 'globalCreatChat' || oldRouteNameStr === 'kbCreatChat') &&
        nameStr !== 'globalCreatChat' &&
        nameStr !== 'kbCreatChat';
...
let scheduledTaskIcon = ref('skill.svg');
...
const getIcon = (path: string) => {
    ...
    const scheduledTasksActiveState = route.name === 'scheduledTaskList';
    ...
    // Skills图标：只在Skills页面显示绿色
    skillIcon.value = skillsActiveState ? 'skill-green.svg' : 'skill.svg';

    // 定时任务图标：闹钟样式，活跃时绿色
    scheduledTaskIcon.value = scheduledTasksActiveState ? 'scheduled-task-green.svg' : 'scheduled-task.svg';
