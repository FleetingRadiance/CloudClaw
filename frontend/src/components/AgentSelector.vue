                ...
                <div class="agent-selector-content">
                    ...
                    <div class="agent-group">
                        ...
                        >
                            ...
                            >
                                <!-- 快速回答和 ReAct 智能体使用图标，其他内置智能体使用 avatar -->
                                <div
                                    v-if="
                                        agent.id === BUILTIN_QUICK_ANSWER_ID ||
                                        agent.id === BUILTIN_SMART_ID
                                    "
                                    class="builtin-icon"
                                ...
                                >
                                    <TIcon
                                        :name="
                                            agent.config?.agent_mode === 'smart-reasoning' ? 'control-platform' : 'chat'
                                        "
                                        size="14px"
                                    />
                                </div>
                                ...
                                <div class="agent-tooltip-content">
                                    ...
                                    <div
                                        ...
                                        <div class="agent-tooltip-title">
                                            ...
                                            <span v-if="isMyAgentSelected(agent)" class="agent-tooltip-selected">{{
                                                $t('agent.selector.current')
                                            }}</span>
                                        </div>
                                    </div>
                                    <p class="agent-tooltip-desc">
                                ...
                                <div class="agent-tooltip-content">
                                    <div class="agent-tooltip-header">
                                        ...
                                        <div class="agent-tooltip-title">
                                            ...
                                            <span v-if="isSharedAgentSelected(shared)" class="agent-tooltip-selected">{{
                                                $t('agent.selector.current')
                                            }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        v-if="shared.org_name || shared.shared_by_username"
                                        class="agent-tooltip-meta-list"
                                    >
                                        <div v-if="shared.org_name" class="agent-tooltip-meta-row">
                                            <img
                                                src="@/assets/img/organization-green.svg"
                                                class="agent-tooltip-meta-icon"
                                                alt=""
                                                aria-hidden="true"
                                            />
...
const builtinAgents = computed(() => {
    // 从 API 获取的内置智能体
    let apiBuiltins = agentsList.value.filter((a) => a.is_builtin);

    // LDAP 用户显示「智能体」和「业务咨询」内置智能体
    if (authStore.user?.user_type === 'ldap_user') {
        apiBuiltins = apiBuiltins.filter((a) => a.id === BUILTIN_SMART_ID || a.id === BUILTIN_WIKI_RESEARCHER_ID);
    }
    ...
    return apiBuiltins.map((agent) => {
        ...
        } else if (agent.id === BUILTIN_SMART_ID) {
            return {
                ...agent,
...
const updateDropdownPosition = () => {
    ...
    if (spaceBelow >= minDropdownHeight + offsetY) {
        ...
        actualHeight =
            availableHeight >= preferredDropdownHeight
