<template>
  <SettingDrawer :visible="dialogVisible" :title="isEdit ? $t('model.editor.editTitle') : $t('model.editor.addTitle')"
    ...
    :confirm-disabled="formData.provider === 'cloudclowcloud' && wkcCredentialState !== 'configured'"
    ...
    <template v-if="formData.source === 'remote'" #footer-left>
      <t-button variant="outline" @click="checkRemoteAPI" :loading="checking"
        :disabled="!formData.modelName || (!formData.baseUrl && formData.provider !== 'cloudclowcloud') || (formData.provider === 'cloudclowcloud' && wkcCredentialState !== 'configured')">
        <template #icon>
    ...
    <t-form ref="formRef" :data="formData" :rules="rules" layout="vertical">
      ...
      <template v-if="formData.source === 'remote'">
        <section class="setting-drawer__section">
          ...
          <!-- CloudClowCloud 提示信息 -->
          <template v-if="formData.provider === 'cloudclowcloud'">
            <!-- 凭证已配置 -->
            <div v-if="wkcCredentialState === 'configured'" class="cloudclowcloud-hint cloudclowcloud-hint--ok">
              <t-icon name="check-circle-filled" class="hint-icon hint-icon--ok" />
              <div>
                {{ $t('settings.cloudclowCloud.modelHintConfigured') }}
                <a href="https://developers.weixin.qq.com/doc/aispeech/knowledge/atomic_capability/atomic_interface.html"
                  ...
                  {{ $t('settings.cloudclowCloud.modelHintDocsLink') }}
            ...
            <!-- 未配置 / 失效 -->
            <div v-else-if="wkcCredentialState !== 'loading'" class="cloudclowcloud-hint cloudclowcloud-hint--warn">
              <t-icon name="error-circle-filled" class="hint-icon hint-icon--warn" />
              <div style="flex: 1;">
                <template v-if="wkcCredentialState === 'expired'">
                  {{ $t('settings.cloudclowCloud.credentialExpired') }}
                </template>
                <template v-else>
                  {{ $t('settings.cloudclowCloud.credentialUnconfigured') }}
                </template>
                <div style="margin-top: 8px;">
                  <t-button variant="text" size="small" @click="goToCloudClowCloudSettings"
                    style="padding: 0; height: auto;">
                    <template #icon><t-icon name="jump" /></template>
                    {{ $t('settings.cloudclowCloud.goToSettings') }}
                  </t-button>
                </div>
              </div>
            </div>
            ...
            <!-- 加载中 -->
            <div v-else class="cloudclowcloud-hint">
              <t-icon name="loading" class="spinning hint-icon hint-icon--loading" />
              <span>{{ $t('settings.cloudclowCloud.checkingStatus') }}</span>
            </div>
          </template>
          ...
          <div class="form-item">
            ...
            <t-input v-model="formData.modelName" :placeholder="getModelNamePlaceholder()"
              :disabled="formData.provider === 'cloudclowcloud' && wkcCredentialState !== 'configured'" />
          </div>
          ...
          <div v-if="formData.provider !== 'cloudclowcloud'" class="form-item">
...
const credentialFields = computed<CredentialFieldDef<ModelCredentialField>[]>(() => {
  ...
  if (formData.value.provider === 'cloudclowcloud') {
...
const checkWkcCredentialStatus = async () => {
  ...
  try {
    const status = await getCloudClowCloudStatus()
...
const goToCloudClowCloudSettings = async () => {
  emit('update:visible', false)
  if (uiStore.showSettingsModal) {
    uiStore.closeSettings()
    await nextTick()
  }
  uiStore.openSettings('cloudclowcloud')
}
...
watch(() => props.visible, (val) => {
  if (val) {
    ...
    try {
      ...
      // 如果当前 provider 是 CloudClowCloud，检查凭证状态
      if (formData.value.provider === 'cloudclowcloud') {
...
const handleProviderChange = (value: string) => {
  ...
  // CloudClowCloud: 检查凭证状态
  if (value === 'cloudclowcloud') {
    checkWkcCredentialStatus()
  }
...
const checkRemoteAPI = async () => {
  if (!formData.value.modelName || (!formData.value.baseUrl && formData.value.provider !== 'cloudclowcloud')) {
...
const handleConfirm = async () => {
  try {
    ...
    // 如果是 remote 类型且非 CloudClowCloud，必须填写 baseUrl
    if (formData.value.source === 'remote' && formData.value.provider !== 'cloudclowcloud') {
